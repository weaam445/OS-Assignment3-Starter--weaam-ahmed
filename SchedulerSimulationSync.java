...
class SharedResources {
    ...
    public static final ReentrantLock competedProcessLock=new ReentrantLock();      
   public static final ReentrantLock  waitingTimeLock=new ReentrantLock();  
    ...
    public static final Semaphore cpuSemaphore =new Semaphore(permits:1);
    ...
    contextSwitchCount.lock();
        try{
            contextSwitchCount++ ;
        }finally{
            contextSwitcLock.unlock();   
        }
    }
...
class Process implements Runnable {
    ...
    public void run() {
        try{
            sharedResources.cpuSmaphore.acquire();
        try {
            if (startTime == -1) {
            ...
            sharedResources.cpuSmaphore.release ();
    }
        }catch (interruptedException e){
            System.out.println(color.Red +"x"+name+"semaphore in
                               }
                               }
